  ...
  private static class Serializer extends AbstractKafkaAvroSerializer {
    ...
    public byte[] serializeSimplifiedImpl(
      ...
      try {
        ...
        Collection<String> subjects =schemaRegistry.getAllSubjectsByPrefix(subject);

        if(subjects.isEmpty() & autoregister){
          System.out.println("metadata null, registering");
          String subjecturl= hortonSRbaseURL+topic+schemaTextUri;
        ...
        String outputFinal;
          ...
          id=schemaRegistry.register(subject,scm2);

          conn.disconnect();
          subjects.clear();
        }
        else
          id=schemaRegistry.getLatestSchemaMetadata(subject).getId();
        if (id==0){
