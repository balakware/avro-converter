...
public class AvroConverterHSRDynamicSchemaId implements Converter {
  ...
  private static final Map<String,Integer> schemaMap=new HashMap<>();
...
//          new MapMaker().weakKeys().makeMap();
  ...
  private static class Serializer extends AbstractKafkaAvroSerializer {
    ...
    public byte[] serializeSimplifiedImpl(
      ...
      byte [] scm=Arrays.copyOfRange(dataBytes, 1, 5);
      int schemaId = new BigInteger(scm).intValue() ;
      if (schemaRegistry == null) {
      ...
      try {
        ...
        id=util.getHsrSchemId(schemaId,hsrScmVerPath,subject,schemaMap,hortonSRbaseURL,autoregister,schemaRegistry);
        schemaMap.put(subject+schemaId,id);
